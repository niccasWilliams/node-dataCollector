 ...
 * Should extract default infos from the variants connected
 */
export const mergedProducts = pgTable("merged_products", {
    ...
    categoryIdx: index("merged_product_category_idx").on(table.category),
...
/**
 * Products table (direct from websites/sources)
 */
 ...
 * Stores the most recent price for each product source
 */
export const productPrices = pgTable("product_prices", {
    ...
    productSourceUnique: unique("product_price_source_unique").on(table.productSourceId), // One current price per source
...
export const priceHistory = pgTable("price_history", {
    ...
    sourceRecordedIdx: index("price_history_source_recorded_idx").on(table.productSourceId, table.recordedAt),
}));

/**
 * Price Alerts Table - Price Drop/Error Alerts
 * Tracks user-defined alerts for price monitoring
 * Alerts are set on merged products (the master product)
 */
...
export const productAttributes = pgTable("product_attributes", {
    ...
    mergedVariantTypeKeyUnique: unique("product_attribute_merged_variant_type_key_unique").on(table.variantId, table.type, table.key),
}));
...
export const productMatchSuggestions = pgTable("product_match_suggestions", {
    ...
    mergedProductUnique: unique("product_match_suggestion_merged_product_unique").on(table.mergedProductId, table.productId),
}));
...
export type ProductMatchSuggestionId = typeof productMatchSuggestions.$inferSelect["id"];
